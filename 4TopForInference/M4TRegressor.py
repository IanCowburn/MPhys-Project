from M4TRegressorDataLoading import TransformerDataLoader
from M4TRegressorTraining import TransformerScaling, TransformerTraining
from M4TRegressorEvaluation import TransformerEvaluation
from M4TRegressorPlotting import TransformerPlotting
...
pad_mask_first_half = pad_mask_np[:len(pad_mask_np) // 2]

X_second_half = X[len(X) // 2:]
y_second_half = y[len(y) // 2:]
pad_mask_second_half = pad_mask_np[len(pad_mask_np) // 2:]

for suffix in ['_model_1', '_model_2']:

    if suffix == '_model_1':
        X = X_first_half
        y = y_first_half
        pad_mask_np = pad_mask_first_half
    else:
        X = X_second_half
        y = y_second_half
        pad_mask_np = pad_mask_second_half
    ...
    }, f'four_top_transformer{suffix}.pt')
    ...
    with open(f'four_top_transformer{suffix}_outputs.pkl', 'wb') as f:
        pickle.dump(model_outputs, f)

    print(f"Model outputs saved to four_top_transformer{suffix}_outputs.pkl")
    ...
    torch.onnx.export(
        ...
        f'four_top_transformer{suffix}.onnx',
        export_params=True,
        opset_version=15,
    ...
    print(f"Model exported to four_top_transformer{suffix}.onnx")

    # Load and test the ONNX model
    ort_session = ort.InferenceSession(f"four_top_transformer{suffix}.onnx")
