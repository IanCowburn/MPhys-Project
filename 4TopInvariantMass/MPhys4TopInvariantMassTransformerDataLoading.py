...
class TransformerDataLoader:
    ...
    def data_combination(self, data_array, combined_parton_system):
        ...
        charge_zeros = -99 * np.ones([data_array.shape[1], self.jet_mask_size])  # Shape: (events, jet_tokens)
        charge_leptons = np.concatenate([lepton_arrays[4, :, :], charge_zeros], axis=1)  # Shape: (events, total_tokens)
        bjet_zeros = -99 * np.ones([data_array.shape[1], self.lepton_mask_size])  # Shape: (events, lepton_tokens)
    ...
    def __call__(self):
        ...
        if os.path.exists("cache_X.npy"):
            X = np.load("cache_X.npy")
            y = np.load("cache_y.npy")
            pad_mask_np = np.load("cache_pad_mask.npy")
            return X, y, pad_mask_np
        ...
        X, y, pad_mask_np = self.final_masking(data_array, combined_parton_system, num_phys_features=6)

        np.save("cache_X.npy", X)
        np.save("cache_y.npy", y)
        np.save("cache_pad_mask.npy", pad_mask_np)
