...
class TransformerPlotting:
    ...
    def plot_all(self):
        ...
        # Ratio (Pred/True)
        counts_true, bin_edges = np.histogram(self.y_true[:,0], bins=bins, range=(x_min, x_max))
        counts_pred, _ = np.histogram(self.y_pred[:,0], bins=bins, range=(x_min, x_max))
        ...
        plt.tight_layout()
        plt.savefig('transformer_dual_regression_true_invariant_mass_distribution.png', dpi=300)
        plt.show()
        plt.close()
        fig = plt.figure(figsize=(10, 8))
        gs = gridspec.GridSpec(nrows=2, ncols=1, height_ratios=[3, 1], hspace=0.05)
        ...
        ax_main.hist(self.y_pred[:,1], bins=bins, range=(x_min, x_max),
                     histtype='step', linewidth=1.5, color='red', label='Predicted')
        ax_main.set_ylabel('Frequency')
        ax_main.set_title('HT Distribution')
        ax_main.grid(True, alpha=0.2)
        ax_main.legend(loc='upper right')
        # Ratio (Pred/True)
        counts_true, bin_edges = np.histogram(self.y_true[:,1], bins=bins, range=(x_min, x_max))
        counts_pred, _ = np.histogram(self.y_pred[:,1], bins=bins, range=(x_min, x_max))
        ratio = np.divide(counts_pred, counts_true, where=counts_true != 0)
        ratio[np.isnan(ratio)] = 0.0
        bin_centers = 0.5 * (bin_edges[:-1] + bin_edges[1:])
        ax_ratio.plot(bin_centers, ratio, color='black', linewidth=1.2)
        ...
        plt.setp(ax_main.get_xticklabels(), visible=False)
        plt.tight_layout()
        plt.savefig('transformer_dual_regression_true_ht_distribution.png', dpi=300)
        plt.show()
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig('transformer_dual_regression_invariant_mass_binned_plot_percent.png', dpi=300)
        plt.show()
        plt.close()
        ...
        cbar.set_ticks([0, H_percent.max()/4, H_percent.max()/2, 3*H_percent.max()/4, H_percent.max()])
        plt.tight_layout()
        plt.savefig('transformer_dual_regression_ht_mass_binned_plot_percent.png', dpi=300)
        plt.show()
        plt.close()
