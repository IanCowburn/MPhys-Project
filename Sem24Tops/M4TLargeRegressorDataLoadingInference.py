...
class TransformerDataLoader:
    ...
    def initial_masking(self, data_array, parton_vectors, tree, parton_pt):
        ...
        jet_counts = ak.num(masking_array["jet_eta"])
        nJets_mask = (jet_counts >= 8) & (jet_counts <= self.jet_mask_size)
    ...
    def __call__(self):
        ...
        """
        if os.path.exists("dual_large_cache_8plus_X.npy"):
            X = np.load("dual_large_cache_8plus_X.npy")
            y = np.load("dual_large_cache_8plus_y.npy")
            pad_mask_np = np.load("dual_large_cache_8plus_pad_mask.npy")
            return X, y, pad_mask_np
        data_array = []
        ...
        np.save("dual_large_cache_8plus_X.npy", X)
        np.save("dual_large_cache_8plus_y.npy", y)
        np.save("dual_large_cache_8plus_pad_mask.npy", pad_mask_np)
        return X, y, pad_mask_np
