...
class TransformerDataLoader:
    ...
    def __call__(self):
        ...
        """
        if os.path.exists("atlas_dual_cache_X.npy"):
            X = np.load("atlas_dual_cache_X.npy")
            y = np.load("atlas_dual_cache_y.npy")
            pad_mask_np = np.load("atlas_dual_cache_pad_mask.npy")
            return X, y, pad_mask_np
        data_array = []
        ...
        np.save("atlas_dual_cache_X.npy", X)
        np.save("atlas_dual_cache_y.npy", y)
        np.save("atlas_dual_cache_pad_mask.npy", pad_mask_np)
        return X, y, pad_mask_np