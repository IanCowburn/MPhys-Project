# Imports
from M4TLargeRegressorModelInference import TransformerRegressor
import pickle
import numpy as np
import torch
...
class TransformerScaling:
    ...
    def save_scalers(self, path):
        if self.feature_scalers is None or self.target_scaler is None:
            raise ValueError("Scalers are not fitted yet. Run __call__ first.")

        scaler_bundle = {
            "feature_scalers": self.feature_scalers,
            "target_scaler": self.target_scaler,
            "layout": {
                "lepton_mask_size": self.lepton_mask_size,
                "jet_mask_size": self.jet_mask_size,
                "in_features_leptons": self.in_features_leptons,
                "in_features_jets": self.in_features_jets,
                "in_features_met": self.in_features_met,
                "in_features_numbers": self.in_features_numbers,
                "in_features_ht": self.in_features_ht,
            },
        }
    ...
    def __call__(self, save_scalers_path=None):
        ...
        self.target_scaler = scaler_y
        if save_scalers_path is not None:
            self.save_scalers(save_scalers_path)
            print(f"Saved scalers to {save_scalers_path}")
...
class TransformerTraining:
    def __init__(self,
                 ...
                 epochs, batch_size,
                 learning_rate=4e-4, kl_weight=0.05, kl_warmup_epochs=15, warmup_epochs=10, num_bins=128, early_stopping_patience=25,
                 pretrained_state_dict=None):  # <-- add
        self.lepton_mask_size = lepton_mask_size
        self.jet_mask_size = jet_mask_size
        self.in_features_leptons = in_features_leptons
        self.in_features_jets = in_features_jets
        ...
        self.pretrained_state_dict = pretrained_state_dict  # <-- add
    ...
    def training_loop(self, model, device, train_loader, valid_loader):
        early = EarlyStopping(patience=self.early_stopping_patience, min_delta=1e-4)
        loss_fn = nn.MSELoss() 
        train_loss_history = []
        val_loss_history = []
        ...
        kl_weight_max = self.kl_weight
        kl_ramp_epochs = self.kl_warmup_epochs
    ...
    def __call__(self):
        ...
        if self.pretrained_state_dict is not None:
            try:
                model.load_state_dict(self.pretrained_state_dict, strict=True)
