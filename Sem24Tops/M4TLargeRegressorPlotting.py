...
class TransformerPlotting:
    def __init__(self, y_true, y_pred, invariant_mass_corr, invariant_mass_rmse, invariant_mass_difference, ht_corr, ht_rmse, ht_difference, invariant_mass_biased, ht_biased, embedder_output, train_loss_history, val_loss_history, suffix):
    ...
    def plot_loss_curves(self, outpath=None):
        if outpath is None:
            outpath = f'four_top_large_transformer{self.suffix}_training_validation_loss.png'
    ...
    def plot_all(self):
        ...
        with plt.rc_context({'font.size': 16}):
            ...
            fig.savefig(f'four_top_large_transformer{self.suffix}_mass_ht_error_distribution.png', dpi=300, bbox_inches='tight')
            plt.show()
            plt.close(fig)
        ...
        with plt.rc_context({'font.size': 16}):
            ...
            ax_ratio_mass.step(bin_centers_mass_plot, ratio_mass, where = "mid", color='black', linewidth=1.2)
            ...
            ax_ratio_ht.step(bin_centers_ht_plot, ratio_ht, where = "mid", color='black', linewidth=1.2)
            ...
            fig.savefig(f'four_top_large_transformer{self.suffix}_mass_ht_distribution.png', dpi=300, bbox_inches='tight')
            plt.show()
            plt.close(fig)
        ...
        with plt.rc_context({'font.size': 20}):
            ...
            fig.savefig(f'four_top_large_transformer{self.suffix}_mass_ht_2d_histogram.png', dpi=300, bbox_inches='tight')
            plt.show()
            plt.close(fig)
        ...
        with plt.rc_context({'font.size': 16}):
            ...
            fig.savefig(f'four_top_large_transformer{self.suffix}_mass_ht_binned_plot_percent.png', dpi=300, bbox_inches='tight')
            plt.show()
            plt.close(fig)
